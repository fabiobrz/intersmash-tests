 ...
 * AMQ Streams (Kafka) instance. Connections are performed both as not secured (plaintext) and secured via SSL with
 * SSLContext configured via Elytron too.
 * <p>
 */
/**
 * Set up an WildFly/EAP s2i application that starts configured (during the app build) server which
 ...
 * In this use case, the Elytron SSL context name is configured <i>per-connection</i>, see the upstream version at
 * https://github.com/wildfly/wildfly/blob/main/testsuite/integration/microprofile/src/test/java/org/wildfly/test/integration/microprofile/reactive/messaging/kafka/ssl/microprofile-config-ssl-connection.properties
 */
public class WildflyMicroProfileReactiveMessagingPerConnectorSecuredApplication
	...
	public static final String APP_NAME = "eap-reactive-messaging";
	...
	public WildflyMicroProfileReactiveMessagingPerConnectorSecuredApplication() {
		...
		// the application is deployed). As such, let's configure its value here via environment properties.
		environmentVariables.add(
